...
def write_report(run_dir: Path) -> Path:
    ...
    if data_profile:
        ...
        enrich = ((data_profile.get("details") or {}).get("enrichments") or {})
        enrich_s = ""
        if isinstance(enrich, dict) and enrich:
            enrich_s = " | Enrichments=" + ",".join(sorted([str(k) for k in enrich.keys()]))
        ...
        parts.append(
            ...
            f"feature_lag_days={features.get('feature_lag_days')}{enrich_s}</p>"
        )

        warnings: list[str] = []
        warnings.extend([str(w) for w in (data_profile.get("warnings") or []) if str(w).strip()])
    ...
    creative_dirs = [run_dir, run_dir / "creative_analysis"]
    creative_summary = None
    creative_neighbors = None
    for cd in creative_dirs:
        if creative_summary is None and (cd / "creative_cluster_summary.csv").exists():
            try:
                creative_summary = pd.read_csv(cd / "creative_cluster_summary.csv")
            except Exception:
                creative_summary = None
        if creative_neighbors is None and (cd / "creative_neighbors.csv").exists():
            try:
                creative_neighbors = pd.read_csv(cd / "creative_neighbors.csv")
            except Exception:
                creative_neighbors = None

    if creative_summary is not None:
        parts.append("<div class='card'><h2>Creative Clusters (Summary)</h2>")
        parts.append("<p class='subtle'>Optional: generated by `elv creative-analyze`.</p>")
        parts.append(_df_to_html(creative_summary, max_rows=20))
        parts.append("</div>")
    if creative_neighbors is not None:
        parts.append("<div class='card'><h2>Creative Similarity (Neighbors)</h2>")
        parts.append("<p class='subtle'>Optional: generated by `elv creative-analyze`.</p>")
        parts.append(_df_to_html(creative_neighbors, max_rows=20))
        parts.append("</div>")
